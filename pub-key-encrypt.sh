...
email=`cat ~/.ssh/id_rsa.pub | awk '{print $3}'`
echo "-- PKCS8 formatted RSA public key for: $email --"

base64_pkcs8_public_key=`cat ~/.ssh/id_rsa.pub.pkcs8 | grep -v \- | cat`
...
base64_rsa_encrypted_nonce=`echo -n $random_nonce | openssl rsautl -encrypt -inkey ~/.ssh/id_rsa.pub.pkcs8 -pubin | base64 --break 64`
...
openssl enc -aes-256-cbc -e -nosalt -pass pass:$random_nonce -in /dev/stdin | base64 --break 67 > /dev/stdout
